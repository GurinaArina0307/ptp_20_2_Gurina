...
namespace Практика_20
{
    public abstract class TravelPackage //путевка(базовый класс)
    {
        ...
        public string NameTravel
        {
            ...
            set
            {
                string error = Validation.ValidateString(value, "name");
        ...
        public int Duration
        {
            ...
            set
            {
                string error = Validation.ValidateInt(value, "Ghjk") + Validation.ValidateDuration(value);
                if (!string.IsNullOrEmpty(error))
                    throw new ArgumentException(error);
                duration = value;
            }
        ...
        public int Price
        {
            ...
            set
            {
                string error = Validation.ValidateInt(value, "BHNJKJ");
                if (!string.IsNullOrEmpty(error))
                    throw new ArgumentException(error);
                price = value;
            }
